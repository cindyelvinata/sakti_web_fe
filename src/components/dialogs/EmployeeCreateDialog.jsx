...
function FilePicker({ label, file, error, accept, disabled, onChange }) {
  ...
  return <div className="mt-5 first:mt-0"><p className="text-[12px] font-semibold text-slate-700">{label}</p><button type="button" onClick={() => inputRef.current?.click()} className="mt-2 flex min-h-[118px] w-full flex-col items-center justify-center rounded-xl border border-dashed border-slate-300 bg-slate-50 px-3 text-center text-slate-400 disabled:opacity-60" disabled={disabled}><ImagePlus size={26} /><span className="mt-2 text-[11px] font-medium">{file ? 'Ganti File' : `Upload ${label}`}</span>{file && <span className="mt-1 max-w-full truncate text-[10px] text-slate-500">{file.name}</span>}</button><input ref={inputRef} type="file" accept={accept} onChange={(event) => onChange(event.target.files?.[0] || null)} className="hidden" disabled={disabled} />{error && <p className="mt-2 text-[11px] font-medium text-[#EF2427]">{error}</p>}</div>
}
...
export default function EmployeeCreateDialog({ open, onClose, onCreate, onRetryUpload, managerOptions = [] }) {
  ...
  const submit = async (event) => {
    ...
    try {
      const result = await onCreate?.(form, files)
