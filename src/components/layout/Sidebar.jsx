...
export default function Sidebar({ open, onClose, onLogout }) {
  ...
  return <>
    <div className={`fixed inset-0 z-30 bg-slate-950/40 transition lg:hidden ${open ? 'opacity-100' : 'pointer-events-none opacity-0'}`} onClick={onClose} />
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-[280px] flex-col border-r border-slate-200 bg-[#F9FAFC] transition-transform lg:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
      <div className="flex h-[77px] items-center justify-between border-b border-slate-200 px-10">
        <Logo />
        <Button className="p-1 lg:hidden" onClick={onClose} aria-label="Tutup menu"><X /></Button>
      </div>
