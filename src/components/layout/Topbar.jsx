...
export default function Topbar({ onMenuClick }) {
  ...
  return <header className="flex h-[77px] items-center justify-between gap-4 border-b border-slate-200 bg-white px-5 sm:px-8 lg:px-12">
    ...
    <div className="flex min-w-0 shrink items-center gap-3">
      <Avatar className="size-10 bg-[#E62727]">{user?.foto_url ? <img src={user.foto_url} alt="" className="size-full object-cover" /> : <img src={personIcon} alt="" className="size-[18px] brightness-0 invert" />}</Avatar>
      <p className="hidden max-w-[220px] truncate text-sm font-semibold text-slate-900 sm:block">{displayName(user)}</p>
    </div>
  </header>
}
