...
export async function getLeaveReport(params) {
  ...
  if (!data?.success) throw new Error(data?.message || 'Gagal memuat laporan cuti.')
...
export async function downloadLeaveReport(params) {
  const response = await apiClient.get('/api/admin/cuti/export', { params, responseType: 'blob' })
  return { file: response.data, contentDisposition: response.headers['content-disposition'] }
}
