...
export async function uploadWorkConfigurationLogo(file) {
  ...
  const { data } = await apiClient.post('/api/upload/image', formData, { headers: { 'Content-Type': 'multipart/form-data' } })

  if (!data?.success) throw new Error(data?.message || 'Gagal mengunggah logo kantor.')
