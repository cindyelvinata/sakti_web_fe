...
function isActiveEmployee(employee) {
  return normalizeStatus(employee?.status_karyawan) === "aktif";
}
...
function signatureUrl(employee) {
  return (
    employee?.tanda_tangan?.url_tanda_tangan ||
    employee?.tanda_tangan?.url ||
    employee?.url_tanda_tangan ||
    employee?.tanda_tangan_url ||
    employee?.ttd_url ||
    ""
  );
}
...
function fileError(file, types, maxSize, label) {
  ...
  if (!types.includes(file.type))
    return `${label} harus berformat ${label === "Foto" ? "jpg, png, jpeg, atau webp" : "png, jpg, atau jpeg"}.`;
...
function StatusBadge({ status }) {
  return (
    <span
      className={cn(
        "inline-flex min-w-[61px] justify-center rounded-full border px-3 py-1 text-[10px] font-bold uppercase",
        status === "Aktif"
          ? "border-[#B7DFE9] bg-[#EDF9FC] text-[#1E93AB]"
...
function FieldInput({ value, onChange }) {
  return (
    <input
      ...
      className="h-[30px] w-full rounded-md border border-slate-200 bg-white px-2 text-[11px] text-slate-700 outline-none focus:border-[#1E93AB]"
    />
  );
}
function FieldSelect({ value, onChange, options }) {
  const normalizedOptions = options.map((option) =>
    typeof option === "string"
      ? { label: titleCase(option), value: option }
      : option,
  );
  const hasValue =
  ...
  const selectOptions = hasValue
    ? normalizedOptions
    : [{ label: titleCase(value), value }, ...normalizedOptions];
  return (
    <select
      value={value ?? ""}
      onChange={(event) => onChange(event.target.value)}
  ...
  return (
    <div className="relative inline-flex items-center">
      <button
        ...
        onClick={() => onToggle(id)}
        className={cn(
          "flex items-center gap-1.5 whitespace-nowrap text-[12px] font-bold uppercase text-[#707989]",
          value && "text-[#EF2427]",
        )}
      >
        {label}
...
function ManagerSelect({ value, onChange, options }) {
  return (
    <select
      value={value ?? ""}
    ...
    >
      ...
      {options.map((manager) => (
        <option key={manager.id} value={manager.id}>
          {manager.nama_lengkap || manager.email || `Atasan ${manager.id}`}
        </option>
      ))}
    </select>
  );
}
function EditFilePicker({
  employeeId,
  kind,
  label,
  currentUrl,
    ...
    <div className="flex flex-col items-center gap-1 text-[9px] text-slate-500">
      ...
      >
        <input
          ...
          disabled={disabled}
          onChange={(event) =>
            onSelect(employeeId, kind, event.target.files?.[0] || null)
          }
        />
        {previewUrl ? (
          <img
            ...
            alt=""
            className={cn(
              "size-10 object-cover ring-1 ring-slate-200 group-hover:ring-[#1E93AB]",
              kind === "photo" ? "rounded-full" : "rounded-md bg-white",
            )}
          />
        ) : (
          <span
            className={cn(
              "grid size-10 place-items-center bg-slate-100 text-[10px] font-bold text-slate-400 ring-1 ring-slate-200 group-hover:ring-[#1E93AB]",
              kind === "photo" ? "rounded-full" : "rounded-md",
            )}
          >
            {label}
          </span>
        ...
        {file && (
          <span className="max-w-[86px] truncate font-semibold text-slate-500 group-hover:text-[#1E93AB]">
            {file.name}
          </span>
        )}
      </label>
      {(error || uploadError) && (
        <p className="max-w-[86px] text-center text-[9px] font-semibold text-[#EF2427]">
          {error || uploadError}
        </p>
      )}
      {uploadError && file && (
        <button
          type="button"
          onClick={() => onRetry(employeeId, kind)}
          disabled={disabled || isRetrying}
          className="flex items-center gap-1 rounded-full border border-[#B7DFE9] bg-[#EDF9FC] px-2 py-1 text-[9px] font-bold text-[#1E93AB] disabled:opacity-60"
        >
          <RefreshCw size={10} />
    ...
    pageCount > 1
      ? await Promise.all(
          Array.from({ length: pageCount - 1 }, (_, index) =>
            getEmployees({ ...params, page: index + 2, limit: 100 }),
          ),
        )
      : [];
...
function buildCreatePayload(form) {
  ...
  if (form.atasan_langsung_id)
    payload.atasan_langsung_id = form.atasan_langsung_id;
...
function toEmployeeRow(employee, nameById) {
  return {
    ...
    managerId:
      employee.atasan_langsung_id != null
        ? String(employee.atasan_langsung_id)
        : "",
...
export default function EmployeeManagementPage() {
    ...
    async (targetPage = page) => {
      ...
      try {
        const result = await getEmployees(
          buildEmployeeParams({ page: targetPage, limit: 10, query, filters }),
        );
  ...
  const loadManagers = useCallback(async () => {
    try {
      const candidates = await getAllEmployees({ status: "aktif" });
      const activeEmployees = candidates.filter(isActiveEmployee);
      setManagerCandidates(
        activeEmployees.length ? activeEmployees : candidates,
      );
    } catch (error) {
      if ([401, 403].includes(error.response?.status)) {
        authStorage.clearSession();
  ...
  const managerOptions = useMemo(() => {
    const byId = new Map();
    [...managerCandidates, ...employees.filter(isActiveEmployee)].forEach(
      (employee) => {
        if (employee?.id != null) byId.set(String(employee.id), employee);
      },
    );
    return [...byId.values()].sort((first, second) =>
      String(first.nama_lengkap || "").localeCompare(
        String(second.nama_lengkap || ""),
      ),
    );
  ...
  const records = useMemo(
    () => employees.map((employee) => toEmployeeRow(employee, nameById)),
    [employees, nameById],
  );
  ...
  const setEditFile = (id, kind, file) => {
    const error = fileError(
      file,
      kind === "photo" ? photoTypes : signatureTypes,
      kind === "signature" ? maxSignatureSize : null,
      kind === "photo" ? "Foto" : "TTD",
    );
    setEditFiles((current) => ({
      ...current,
      [id]: { ...current[id], [kind]: error ? null : file },
    }));
    setEditFileErrors((current) => ({
      ...current,
      [id]: { ...current[id], [kind]: error },
    }));
    setEditUploadErrors((current) => ({
      ...current,
      [id]: { ...current[id], [kind]: "" },
    }));
  };
  ...
  const toUpdatePayload = (draft, original) => {
    ...
    if (draft.position !== original.position)
      payload.level_jabatan = draft.position;
    if (draft.division !== original.division) payload.divisi = draft.division;
    if (draft.unit !== original.unit) payload.unit = draft.unit;
    if (normalizeStatus(draft.status) !== normalizeStatus(original.status))
      payload.status_karyawan = normalizeStatus(draft.status);
    if (String(draft.managerId || "") !== String(original.managerId || ""))
      payload.atasan_langsung_id = draft.managerId || null;
    return payload;
  };
  const save = async () => {
    setSaveError("");
    ...
    try {
      ...
      const updates = draftEmployees
        .map((draft) => ({ draft, original: originalById.get(draft.id) }))
        .filter(({ original }) => original)
        .map(({ draft, original }) => ({
          id: draft.id,
          payload: toUpdatePayload(draft, original),
          draft,
          original,
        }))
        .filter(({ payload }) => Object.keys(payload).length > 0);
      await Promise.all(
        updates.map(({ id, payload }) => updateEmployee(id, payload)),
      );
      const selectedUploads = Object.entries(editFiles).flatMap(([id, files]) =>
        ["photo", "signature"]
          .filter((kind) => files?.[kind])
          .map((kind) => ({ id, kind, file: files[kind] })),
      );
      if (selectedUploads.length) setSaveLabel("Mengupload...");
      const nextUploadErrors = {};
      await Promise.all(
        selectedUploads.map(async ({ id, kind, file }) => {
          try {
            await uploadEditFile(id, kind, file);
          } catch (error) {
            nextUploadErrors[id] = {
      ...
      if (Object.keys(nextUploadErrors).length) {
        ...
        setEditFiles((current) =>
          Object.fromEntries(
            Object.entries(current).map(([id, files]) => [
              id,
              {
                photo: nextUploadErrors[id]?.photo ? files.photo : null,
                signature: nextUploadErrors[id]?.signature
                  ? files.signature
                  : null,
              },
            ]),
        ...
        setSaveError(
          "Perubahan data tersimpan, tetapi ada file yang gagal diupload. Silakan retry file tersebut.",
        );
        return;
      }
      setDraftEmployees([]);
  ...
  const retryCreateUpload = async (karyawanId, files) => ({
    karyawanId,
    uploadErrors: await uploadCreateFiles(karyawanId, files, 1),
  });
  const downloadCsv = async () => {
    ...
    try {
      const filteredEmployees = uniqueById(
        await getAllEmployees(
          buildEmployeeParams({ page: 1, limit: 100, query, filters }),
        ),
      );
      ...
      const exportNameById = new Map(
        [...managerOptions, ...filteredEmployees].map((employee) => [
          String(employee.id),
          employee.nama_lengkap,
        ]),
      );
      const exportRows = filteredEmployees.map((employee) =>
        toEmployeeRow(employee, exportNameById),
      );
      const headers = [
        "Nama",
        "No. Telp",
      ...
      const csvRows = exportRows.map((employee) =>
        [
          employee.name,
          employee.phone,
          employee.email,
      ...
      setExportMessage(
        `Berhasil mengekspor ${exportRows.length} data karyawan.`,
      );
    } catch (error) {
      if ([401, 403].includes(error.response?.status)) {
        authStorage.clearSession();
  ...
  return (
    <div
      className={cn("mx-auto", isEditMode ? "max-w-none" : "max-w-[1040px]")}
    >
      <header className="mb-9">
        ...
        <p className="mt-2 text-[14px] text-slate-500">
          Kelola data karyawan yang terdaftar dalam sistem
        </p>
      </header>
      <section className="rounded-2xl bg-[#EF2427] p-5">
        <div className="flex flex-col gap-3 lg:flex-row">
          ...
          <button
            ...
            className="flex h-9 items-center justify-center gap-2 rounded-full bg-white px-6 text-[13px] font-medium text-black disabled:opacity-60 lg:w-[214px]"
          >
            <Download size={16} />
            {isExporting ? "Menyiapkan..." : "Unduh CSV"}
          </button>
        </div>
      </section>
      <section className="mt-9 overflow-hidden rounded-2xl border border-slate-200 bg-white">
        ...
        {errorMessage ? (
          ...
          <div className="overflow-x-auto">
            {isEditMode ? (
              <table className="w-full min-w-[1740px] text-left">
                <colgroup>
                ...
                <thead className="bg-[#F7F8FA] text-[12px] font-bold uppercase text-[#707989]">
                  <tr>
                    <th className="px-3 py-4 whitespace-nowrap">Foto</th>
                    <th className="px-3 py-4 whitespace-nowrap">TTD</th>
                    <th className="px-3 py-4 whitespace-nowrap">Nama</th>
                    <th className="px-3 py-4 whitespace-nowrap">No. Telp</th>
                    ...
                    <th className="px-3 py-4 whitespace-nowrap">
                      <ColumnFilter
                        ...
                        onChange={(value) =>
                          updateColumnFilter("level_jabatan", value)
                        }
                      />
                    </th>
                    <th className="px-3 py-4 whitespace-nowrap">Atasan</th>
                    <th className="px-3 py-4 whitespace-nowrap">
                      <ColumnFilter
                        id="divisi"
                        label="Divisi"
                        value={filters.divisi}
                        options={divisionFilters}
                        openFilter={openFilter}
                        onToggle={(id) =>
                          setOpenFilter((current) => (current === id ? "" : id))
                        }
                        onChange={(value) =>
                          updateColumnFilter("divisi", value)
                        }
                      />
                    </th>
                    <th className="px-3 py-4 whitespace-nowrap">
                      <ColumnFilter
                        ...
                        onChange={(value) => updateColumnFilter("unit", value)}
                      />
                    </th>
                    <th className="px-3 py-4 whitespace-nowrap">
                      <ColumnFilter
                    ...
                    <tr key={employee.id} className="border-t border-slate-200">
                      <td className="px-3 py-2">
                        <EditFilePicker
                          ...
                          file={editFiles[employee.id]?.photo}
                          error={editFileErrors[employee.id]?.photo}
                          uploadError={editUploadErrors[employee.id]?.photo}
                          isRetrying={
                            retryingEditUpload === `${employee.id}:photo`
                          }
                          disabled={isSaving || Boolean(retryingEditUpload)}
                      ...
                      <td className="px-3 py-2">
                        <EditFilePicker
                          ...
                          kind="signature"
                          label="TTD"
                          currentUrl={employee.signature}
                          file={editFiles[employee.id]?.signature}
                          error={editFileErrors[employee.id]?.signature}
                          uploadError={editUploadErrors[employee.id]?.signature}
                          isRetrying={
                            retryingEditUpload === `${employee.id}:signature`
                          }
                          disabled={isSaving || Boolean(retryingEditUpload)}
                          onSelect={setEditFile}
                          onRetry={retryEditUpload}
                      ...
                      <td className="px-3 py-2">
                        <FieldInput
                          value={employee.name}
                          onChange={(value) =>
                            updateDraft(employee.id, "name", value)
                          }
                        />
                      </td>
                      <td className="px-3 py-2">
                        <FieldInput
                          ...
                          onChange={(value) =>
                            updateDraft(employee.id, "phone", value)
                          }
                        />
                      </td>
                      <td className="px-3 py-2">
                        <FieldInput
                          ...
                          onChange={(value) =>
                            updateDraft(employee.id, "email", value)
                          }
                        />
                      </td>
                      <td className="px-3 py-2">
                        <FieldSelect
                          value={employee.role}
                          onChange={(value) =>
                            updateDraft(employee.id, "role", value)
                          }
                          options={roles}
                        />
                      </td>
                      <td className="px-3 py-2">
                        <FieldSelect
                          ...
                          onChange={(value) =>
                            updateDraft(employee.id, "position", value)
                          }
                          options={positions}
                        />
                      </td>
                      <td className="px-3 py-2">
                        <ManagerSelect
                          ...
                          onChange={(value) =>
                            updateDraft(employee.id, "managerId", value)
                          }
                          options={managerOptions.filter(
                            (manager) =>
                              String(manager.id) !== String(employee.id),
                          )}
                        />
                      </td>
                      <td className="px-3 py-2">
                        <FieldInput
                          ...
                          onChange={(value) =>
                            updateDraft(employee.id, "division", value)
                          }
                        />
                      </td>
                      <td className="px-3 py-2">
                        <FieldInput
                          value={employee.unit}
                          onChange={(value) =>
                            updateDraft(employee.id, "unit", value)
                          }
                        />
                      </td>
                      <td className="px-3 py-2">
                        <FieldSelect
                          value={employee.status.toLowerCase()}
                          onChange={(value) =>
                            updateDraft(employee.id, "status", titleCase(value))
                          }
              ...
              <table className="w-full min-w-[1644px] text-left">
                <colgroup>
                  <col className="w-[96px]" />
                  <col className="w-[190px]" />
                  <col className="w-[155px]" />
                        ...
                        <td className="px-3 py-3">
                          <div className="flex flex-col items-center text-[9px] text-slate-400">
                            {employee.photo ? (
                              ...
                              <span className="size-10 rounded-full bg-slate-100 ring-1 ring-slate-200" />
                            )}
                          </div>
                        </td>
                        <td className="px-3 py-4 font-bold uppercase">
                          {employee.name}
                        </td>
      ...
      {!errorMessage && !isEditMode && totalPages > 1 && (
        <div className="mt-4 flex flex-wrap items-center justify-center gap-2 text-sm">
          <button
            type="button"
            disabled={currentPage <= 1}
            onClick={() => setPage((current) => Math.max(1, current - 1))}
            className="rounded-full px-3 py-2 font-semibold text-[#1E93AB] transition hover:bg-[#EDF9FC] disabled:pointer-events-none disabled:text-slate-300 disabled:opacity-70"
          >
            Sebelumnya
          </button>
              ...
              <button
                ...
                className={cn(
                  ...
                  Number(currentPage) === item
                    ? "bg-[#EF2427] text-white shadow-sm"
                    : "text-[#1E93AB] hover:bg-[#EDF9FC]",
                )}
              >
                {item}
              </button>
        ...
        {isEditMode ? (
          <>
            <button
              ...
              onClick={save}
              disabled={isSaving}
              className="h-10 min-w-[140px] rounded-full bg-[#EF2427] px-7 text-[13px] font-bold text-white disabled:opacity-50"
            >
              {saveLabel || "Simpan"}
            </button>
            <button
              ...
              onClick={cancel}
              disabled={isSaving}
              className="h-10 min-w-[140px] rounded-full border border-[#FFB1B1] bg-[#FDE8E8] px-7 text-[13px] font-bold text-[#EF2427] disabled:opacity-50"
            >
              Batal
            </button>
          </>
          ...
          <>
            <button
              ...
              disabled={isLoading}
              className="flex h-10 items-center gap-3 rounded-full bg-[#EF2427] px-7 text-[13px] font-bold text-white disabled:opacity-50"
            >
              <Plus size={16} />
              Tambah Karyawan
            </button>
      ...
      {exportMessage && (
        <p className="mt-3 text-right text-sm font-medium text-[#1E93AB]">
          {exportMessage}
        </p>
      )}
      {saveError && (
        <p className="mt-3 text-right text-sm font-medium text-[#EF2427]">
          {saveError}
        </p>
      )}
      <EmployeeCreateDialog
        ...
        onCreate={create}
        onRetryUpload={retryCreateUpload}
