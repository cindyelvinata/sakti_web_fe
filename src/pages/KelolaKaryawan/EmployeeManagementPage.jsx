...
export default function EmployeeManagementPage() {
  ...
  const loadEmployees = useCallback(async (targetPage = page) => {
    ...
    try {
      const result = await getEmployees({ page: targetPage, limit: 10, ...(query.trim() ? { search: query.trim() } : {}), ...(status === 'Semua Status' ? {} : { status: status.toLowerCase() }) })
