...
export default function EmployeeManagementPage() {
  ...
  const records = useMemo(() => employees.map((employee) => ({ id: employee.id, photo: employee.foto_url, name: employee.nama_lengkap || '-', phone: employee.nomor_telepon || '', email: employee.email || '', role: employee.role || '', position: employee.level_jabatan || '', managerId: employee.atasan_langsung_id != null ? String(employee.atasan_langsung_id) : '', manager: nameById.get(String(employee.atasan_langsung_id)) || '', division: employee.divisi || '-', unit: employee.unit || '-', status: titleCase(employee.status_karyawan) })).sort((first, second) => statusOrder(first.status) - statusOrder(second.status) || first.name.localeCompare(second.name)), [employees, nameById])
  ...
  const downloadCsv = () => {
    ...
    const csvRows = rows.map((employee) => [employee.name, employee.phone, employee.email, employee.role, positionLabel(employee.position), employee.manager, employee.division, employee.unit, employee.status].map(csvValue).join(','))
