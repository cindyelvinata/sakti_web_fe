...
const displayStatus = { cuti: 'Cuti', terlambat: 'Terlambat', tepat_waktu: 'Tepat Waktu', belum_presensi: 'Belum Presensi', presensi_keluar: 'Keluar', lembur: 'Lembur', dalam_radius: 'Dalam Radius', di_luar_radius: 'Di Luar Radius' }
...
function ColumnFilter({ id, label, value, options, openFilter, onToggle, onChange }) {
  ...
  return <div className="relative inline-flex">
    ...
    {openFilter === id && <div className="absolute left-0 top-full z-30 mt-2 w-[180px] rounded-2xl bg-white p-1.5 text-left normal-case shadow-xl">
...
export default function AttendanceReportPage() {
  ...
  const loadReport = useCallback(async () => {
    ...
    try {
      setReport(await getAttendanceReport({
        page,
        limit: 10,
  ...
  const exportReport = async (selection) => {
    ...
    try {
      const preview = await getAttendanceReport({ ...params, page: 1, limit: 1 })
      if (!preview.items.length) {
        setDownloadError('Tidak ada data untuk diunduh.')
        return
      }

      const { file, contentDisposition } = await downloadAttendanceReport(params)
