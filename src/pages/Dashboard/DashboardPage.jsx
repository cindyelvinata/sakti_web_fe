...
const emptyLeaveData = () =>
  leaveStatuses.map((status) => ({ status, total: 0 }));
...
function normalizeAggregates(records, sourceLabel, labelKey) {
  ...
  return records.map((record) => ({
    [labelKey]:
      record?.[sourceLabel] || record?.[labelKey] || "Tidak diketahui",
...
function getAttendanceRange(period) {
  const selected =
    attendancePeriods.find((item) => item.value === period) ??
    attendancePeriods[0];
...
function initialAttendancePeriod() {
  return isCurrentMonthRange(getAttendanceRange("this_week"))
    ? "this_week"
    : "custom";
}
...
function isLeaveRecordInRange(record, { startDate, endDate }) {
  const date = parseLocalDate(
    record?.tanggal_mulai || record?.tanggal || record?.created_at,
  );
  return date ? date >= startDate && date <= endDate : false;
}
...
function totalPages(meta) {
  return Math.max(
    1,
    Number(meta?.total_pages ?? meta?.totalPages ?? meta?.last_page ?? 1) || 1,
  );
}
...
function aggregateAttendance(records) {
  ...
  records.forEach((record) => {
    ...
    if (checkInStatus === "terlambat") inItem.terlambat += 1;
      ...
      checkInStatus === "belum_presensi" ||
      (!record.jam_masuk && !record.jenis_cuti)
    )
      inItem.belumPresensi += 1;
    else if (record.jam_masuk || checkInStatus === "tepat_waktu")
      inItem.tepatWaktu += 1;

    if (checkOutStatus === "lembur" || checkOutStatus === "presensi_lembur")
      outItem.presensiLembur += 1;
    else if (
      checkOutStatus === "belum_presensi" ||
      (!record.jam_keluar && !record.jenis_cuti)
    )
      outItem.belumPresensi += 1;
    else if (
      record.jam_keluar ||
      checkOutStatus === "presensi_keluar" ||
      checkOutStatus === "keluar"
    )
      outItem.presensiKeluar += 1;
  });
...
function attendanceKpi(records) {
  return records.reduce(
    (result, record) => {
      const checkInStatus = statusKey(record.status_masuk || record.jenis_cuti);
      const checkOutStatus = statusKey(
        record.status_keluar || record.jenis_cuti,
      );
...
function approvedLeaveTotal(records) {
  return records.filter((record) => statusKey(record.status) === "disetujui")
    .length;
}

function AttendancePeriodFilter({
  value,
  options,
  customDate,
  rangeLabel,
    ...
    <div className="flex min-w-0 flex-col gap-2 sm:items-end">
      <div className="relative inline-flex max-w-full">
        <select
          ...
          className="h-9 max-w-full appearance-none rounded-full border border-slate-200 bg-white py-0 pr-8 pl-4 text-[12px] font-medium text-slate-700 outline-none focus:border-[#1E93AB]"
        >
          {options.map((period) => (
      ...
      {isCustom ? (
        <input
          ...
          className="h-9 min-w-0 rounded-full border border-slate-200 bg-white px-3 text-[12px] font-medium text-slate-700 outline-none focus:border-[#1E93AB]"
        />
      ) : null}
...
export default function DashboardPage() {
  ...
  const [customAttendanceDate, setCustomAttendanceDate] = useState(() =>
    dateParam(new Date()),
  );
  const [leaveMonth, setLeaveMonth] = useState(() => monthValue(new Date()));
  const [attendanceData, setAttendanceData] = useState(() => ({
    inData: emptyAttendanceIn(),
    outData: emptyAttendanceOut(),
  }));
  const [leavePeriodData, setLeavePeriodData] = useState(() =>
    emptyLeaveData(),
  );
  const [attendancePeriodKpi, setAttendancePeriodKpi] = useState({
    terlambat: 0,
    lembur: 0,
  });
  ...
  const availableAttendancePeriods = useMemo(
    () => [
      ...attendancePeriods
        .map((period) => ({
          ...period,
          range: getAttendanceRange(period.value),
        }))
  ...
  const attendanceRange = useMemo(() => {
    if (attendancePeriod === customAttendancePeriod.value)
      return getCustomAttendanceRange(customAttendanceDate);
  ...
  const loadAttendance = useCallback(async () => {
    ...
    try {
      const params = {
        limit: 100,
        start_date: dateParam(attendanceRange.startDate),
        end_date: dateParam(attendanceRange.endDate),
      };
        ...
        pageCount > 1
          ? await Promise.all(
              Array.from({ length: pageCount - 1 }, (_, index) =>
                getAttendanceReport({ ...params, page: index + 2 }),
              ),
            )
          : [];
      const records = [firstPage, ...otherPages].flatMap(
        (report) => report.items,
      );
      const periodRecords = records.filter((record) =>
        isRecordInRange(record, attendanceRange),
      );
  ...
  const loadLeave = useCallback(async () => {
    ...
    try {
      const params = {
        limit: 100,
        start_date: dateParam(leaveRange.startDate),
        end_date: dateParam(leaveRange.endDate),
      };
      ...
      const otherPages =
        pageCount > 1
          ? await Promise.all(
              Array.from({ length: pageCount - 1 }, (_, index) =>
                getLeaveReport({ ...params, page: index + 2 }),
              ),
            )
          : [];
      const records = [firstPage, ...otherPages].flatMap(
      ...
      const periodRecords = records.filter((record) =>
        isLeaveRecordInRange(record, leaveRange),
      );
  ...
  const statistics = dashboard
    ? [
        ...
        { label: "Karyawan Aktif", value: dashboard.karyawan_aktif },
        {
          ...
          value: isAttendanceLoading ? "..." : attendancePeriodKpi.terlambat,
        },
        {
          label: "Total Lembur",
          value: isAttendanceLoading ? "..." : attendancePeriodKpi.lembur,
        },
        {
          label: "Total Cuti Disetujui",
          value: isLeaveLoading ? "..." : leavePeriodKpi.disetujui,
        },
      ]
    : [];
  ...
  const renderAttendancePeriodFilter = () => (
  ...
  const leavePeriodFilter = (
    <MonthPeriodFilter
      value={leaveMonth}
      options={leaveMonthOptions}
      onChange={setLeaveMonth}
    />
  );
    ...
    <>
      ...
      <section className="grid min-w-0 grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        {isLoading ? (
          Array.from({ length: 5 }, (_, index) => (
            <StatisticCard key={index} label="Memuat data" value="..." />
          ))
        ) : dashboardError ? (
          <div className="rounded-[14px] border border-red-100 bg-red-50 p-5 text-sm text-[#EF2427] sm:col-span-2 lg:col-span-3 xl:col-span-5">
            <p>{dashboardError}</p>
            <button
              type="button"
          ...
          statistics.map((item) => <StatisticCard key={item.label} {...item} />)
        )}
      </section>
      ...
      <section className="mt-6 grid min-w-0 grid-cols-1 gap-6 xl:grid-cols-2">
        <ChartCard
          ...
          isLoading={isLoading}
          isEmpty={!isLoading && !dashboardError && departmentData.length === 0}
        ...
        <ChartCard
          ...
          onRetry={loadLeave}
          action={leavePeriodFilter}
        >
          <LeaveBarChart data={leaveData} />
        </ChartCard>
        <ChartCard
          title="Presensi Masuk berdasarkan Status"
          isLoading={isAttendanceLoading}
          isEmpty={false}
          errorMessage={attendanceError}
          onRetry={loadAttendance}
          action={renderAttendancePeriodFilter()}
        >
          <VerticalBarChart
            data={attendanceInData}
            series={attendanceInSeries}
          />
        </ChartCard>
        <ChartCard
          title="Presensi Keluar berdasarkan Status"
          isLoading={isAttendanceLoading}
          isEmpty={false}
          errorMessage={attendanceError}
          onRetry={loadAttendance}
        ...
        >
          <VerticalBarChart
            data={attendanceOutData}
            series={attendanceOutSeries}
          />
        </ChartCard>
      </section>
    </>
